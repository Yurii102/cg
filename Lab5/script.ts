...
interface ShapeData {
    ...
    fillColor: string;
    originalPoints?: [number, number][]; // Додаємо для зберігання оригінальних точок кожної фігури для анімації
}
...
window.onload = () => {
    ...
    // --- Анімація ---
    const animateBtn = document.getElementById("animateBtn")! as HTMLButtonElement;
    const pauseResumeBtn = document.getElementById("stopAnimBtn")! as HTMLButtonElement; // Перейменовуємо для ясності

    let scaleX_anim: number, scaleY_anim: number, duration_anim: number, mirrorVertexIndex_anim: number;
    ...
    function animateStep(timestamp: number) {
        ...
        if (isPaused) { // Якщо на паузі, нічого не робимо, але зберігаємо frameId для можливого скасування
            if (animationFrameId) cancelAnimationFrame(animationFrameId); // Скасувати попередній запит, якщо він був
        ...
        if (!segmentStartTime) segmentStartTime = timestamp; // segmentStartTime встановлюється при старті/resume
        ...
        drawnShapes.forEach((shape, shapeIndex) => {
            if (!shape.originalPoints) return;
            ...
            const transformedPoints: [number, number][] = currentOriginalPoints.map(([x, y]) => {
    ...
    let segmentStartTime: number | null = null; // Час початку поточного сегменту анімації
    ...
    animateBtn.onclick = () => {
        ...
        scaleX_anim = parseFloat((document.getElementById("animScaleX") as HTMLInputElement).value);
        scaleY_anim = parseFloat((document.getElementById("animScaleY") as HTMLInputElement).value);
        duration_anim = parseFloat((document.getElementById("animDuration") as HTMLInputElement).value);
        mirrorVertexIndex_anim = parseInt((document.getElementById("mirrorVertex") as HTMLSelectElement).value);
        ...
        if (animationFrameId) cancelAnimationFrame(animationFrameId); // На випадок швидких кліків
        animationFrameId = requestAnimationFrame(animateStep);
    };
